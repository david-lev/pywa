...
def register_routes_starlette(wa: WhatsApp):
    ...
    server: StarletteApp = wa.server  # type: ignore
...
def register_routes_fastapi(
    ...
    server: fastapi.FastAPI = wa.server  # type: ignore

    @server.get(wa.webhook_endpoint)
    ...
    @server.post(wa.webhook_endpoint)
...
def register_routes_flask(
    ...
    server: flask.Flask = wa.server  # type: ignore
    ...
    if wa._async_allowed:
        ...
        @server.route(wa.webhook_endpoint, methods=["GET"])
        @rename_func(f"('{wa.webhook_endpoint}')")
        ...
        @server.route(wa.webhook_endpoint, methods=["POST"])
        @rename_func(f"('{wa.webhook_endpoint}')")
...
from .handlers import EncryptedFlowRequestType, FlowRequestCallbackWrapper  # noqa: E402
...
def register_flow_endpoint_starlette(
    ...
    async def pywa_flow(
        ...
        response, status_code = (
            await callback_wrapper.handle_async(await req.json())
            if wa._async_allowed
            else callback_wrapper.handle(await req.json())
        )
        return StarletteResponse(
            content=response,
...
def register_flow_endpoint_fastapi(
    ...
    if wa._async_allowed:
        ...
        async def pywa_flow(
            ...
            """Automatically generated by pywa to handle incoming flow requests."""
            response, status_code = await callback_wrapper.handle_async(flow_req)
            return fastapi.Response(
                content=response,
                status_code=status_code,
            )
